...
contraste_a_multiplicar = 1.6; %Siempre positivo si <0 reduce, si >0 amuenta
...
for i = 1:4
    blood_filtered_4 = blood_filtered_4 + imread(strcat(ruta,'blood_gaussian',num2str(i),'.bmp'))*1/4;
end
% Filtro con 8 imágenes
blood_filtered_8 = uint8(zeros(len(1), len(2)));
for i = 1:8
    blood_filtered_8 = blood_filtered_8 + imread(strcat(ruta,'blood_gaussian',num2str(i),'.bmp'))*1/8;
end
% Filtro con 16 imágenes
blood_filtered_16 = uint8(zeros(len(1), len(2)));
for i = 1:16
    blood_filtered_16 =blood_filtered_16 + imread(strcat(ruta,'blood_gaussian',num2str(i),'.bmp'))*1/16;
end
...
bazo = im2double(ind2gray(image,map));
contrast = max(bazo,[],'all')-min(bazo,[],'all');
display(strcat('El contraste de la imagen original es: ', num2str(contrast),' (contraste máximo = 1)'))

bazo_dark = imadjust(bazo,[min(bazo,[],'all') max(bazo,[],'all')], [0 (max(bazo,[],'all')-min(bazo,[],'all'))]);
contrast = max(bazo_dark,[],'all')-min(bazo_dark,[],'all');
display(strcat('El contraste de la imagen oscurecida es: ', num2str(contrast),' (contraste máximo = 1)'))
% Una gamma alta genera una imagen oscura
bazo_high_gamma = imadjust(bazo,[0 1], [0,1], 2);
contrast = max(bazo_high_gamma,[],'all')-min(bazo_high_gamma,[],'all');
display(strcat('El contraste de la imagen con gamma alto es: ', num2str(contrast),' (contraste máximo = 1)'))
...
display(strcat('El contraste de la imagen con gamma bajo es: ', num2str(contrast),' (contraste máximo = 1)'))
%La gamma afecta al contraste de modo que cuando nos hacercamos a los
%extremos del histograma lo reduce y en el centro del histograma lo aumenta

figure('Name', 'EJ17')
subplot(4,2,1);imshow(bazo);title('original')
subplot(4,2,2);imhist(bazo);title('histograma original')
subplot(4,2,3);imshow(bazo_dark);title('imagen oscurecida')
subplot(4,2,4);imhist(bazo_dark);title('histograma oscirecida')
subplot(4,2,5);imshow(bazo_high_gamma);title('alta gamma')
subplot(4,2,6);imhist(bazo_high_gamma);title('histograma alta gamma')
subplot(4,2,7);imshow(bazo_low_gamma);title('baja gamma')
subplot(4,2,8);imhist(bazo_low_gamma);title('histograma baja gamma')
%--------------------------------------------------------------------------
%%
...
%--------------------------------------------------------------------------
%         18.2.	Compruebe los principales parámetros de la imagen con el 
% comando imfinfo.
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
pout_eq=histeq(pout,25);
subplot(4,2,3);imshow(pout_eq);title('imagen equalizada')
subplot(4,2,4);imhist(pout_eq);title('histograma equalizado')
...
%--------------------------------------------------------------------------
%         18.6.	Sobre la imagen original: Optimice el contraste al máximo, 
% y muestre el histograma de la imagen de salida y su FFT.
%--------------------------------------------------------------------------
pout_contrast = imadjust(pout,[min(pout, [], 'all') max(pout, [], 'all')], [0,1]);
...
display(strcat('El contraste optimizado es: ', num2str(contrast_c),' (contraste máximo = 1)'))
%--------------------------------------------------------------------------
%         18.7.	Añada ruido gaussiano, muestre el histograma de la imagen 
...
%--------------------------------------------------------------------------
figure('Name', 'EJ18.2')
% Es un filtro adaptativo por lo que mantiene bordes y otras partes de alta 
% frecuencia de una imagen. 
% Se adapta a la varianza local de la imagen.
% Donde la varianza es mayor, el filtro Wiener realiza un suavizado menor.
% Donde la varianza es menor, realiza un suavizado mayor. 
...
subplot(4,2,3);imshow(low_s_filter);title('imagen con filtro paso bajo en espacio')
subplot(4,2,4);imhist(low_s_filter);title('histograma con filtro paso bajo en espacio')
%--------------------------------------------------------------------------
...
low_f_filter=low_f_filter(1:rows,1:1:cols);
subplot(4,2,5);imshow(low_f_filter);title('imagen con filtro paso bajo en frecuencuia')
subplot(4,2,6);imhist(low_f_filter);title('histograma con filtro bajo en frecuencuia')
%--------------------------------------------------------------------------
% *    un filtro gaussiano. Genérelo con fspecial y pruebe con diferentes valores 
% de los parámetros.
%--------------------------------------------------------------------------
% Cuanto menor sea la desviación estándar menos se modificará la imagen,
% con una desviación muy alta la "hendidura" que aparece en el histograma
% es mayor.
